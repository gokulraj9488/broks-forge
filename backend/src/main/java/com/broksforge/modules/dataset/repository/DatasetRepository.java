...
public interface DatasetRepository extends JpaRepository<Dataset, UUID>, JpaSpecificationExecutor<Dataset> {
    ...
    /** All non-deleted datasets in an organization (read-only; used to assemble the engineering graph). */
    List<Dataset> findByOrganizationIdAndDeletedFalse(UUID organizationId);

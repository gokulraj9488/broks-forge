...
public enum ErrorCode {
    ...
    DATASET_ARCHIVED(HttpStatus.CONFLICT),
    PROMPT_ARCHIVED(HttpStatus.CONFLICT),
    CREDENTIAL_ENCRYPTION_ERROR(HttpStatus.INTERNAL_SERVER_ERROR),

    // --- Provider Registry (Provider abstraction milestone) ---
    PROVIDER_CONFIG_INVALID(HttpStatus.BAD_REQUEST),
    PROVIDER_IN_USE(HttpStatus.CONFLICT),
    PROVIDER_DISABLED(HttpStatus.CONFLICT),
    ...
    DATASET_UPLOAD_UNSUPPORTED_FORMAT(HttpStatus.BAD_REQUEST),
    DATASET_UPLOAD_TOO_LARGE(HttpStatus.BAD_REQUEST),
    DATASET_UPLOAD_NOT_RETRYABLE(HttpStatus.CONFLICT),
